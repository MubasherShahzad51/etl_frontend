...
const MetricCard = ({ 
  ...
  return (
    <div className={`group relative overflow-hidden bg-white ${isCompact ? 'rounded-xl p-3' : 'rounded-2xl p-6'} border border-gray-100 
    ...
    >
      ...
      <div className="relative flex items-start justify-between">
        <div className="flex-1">
          <p className={`${isCompact ? 'text-[11px]' : 'text-sm'} font-medium text-gray-500 mb-1`}>{title}</p>
          <p className={`${isCompact ? 'text-[16px]' : 'text-3xl'} font-bold text-gray-900 tracking-tight`}>{value}</p>
        ...
        {Icon && (
          <div className={`${isCompact ? 'p-2 rounded-xl' : 'p-3.5 rounded-2xl'} ${iconBg} transition-transform duration-300 group-hover:scale-110 group-hover:rotate-3`}>
      ...
      <div className={`absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r ${
        trend > 0 ? 'from-emerald-500 to-emerald-400' : 
